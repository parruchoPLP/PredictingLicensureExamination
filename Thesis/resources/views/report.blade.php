            ...
            <table class="min-w-full bg-white text-md border border-collapse">
                ...
                <tbody class="text-gray-700">
                    ...
                    <tr class="border-b group hover:bg-emerald-100">
                        ...
                        <td class="py-2 px-4 whitespace-nowrap @if($loop->first) sticky left-0 z-10 bg-white group-hover:bg-emerald-100 @endif @if($loop->last) sticky right-0 z-10 bg-white group-hover:bg-emerald-100 {{ $key == 'predicted_licensure_outcome' && $value == 'Pass' ? 'bg-emerald-100 text-black' : ($key == 'predicted_licensure_outcome' && $value == 'Fail' ? 'bg-red-100 text-black' : '') }}@endif">{{$value}}</td>
                        @endforeach
                    </tr>
                    @endforeach
                </tbody>
